# 2d-mhd
# remove the projection w, H so cross helicity is not preserved
...
# (u, P, u_b, B, E, j)
Z = MixedFunctionSpace([Vc, Q, Vc, Vd, Q, Q])
z = Function(Z)
z_test = TestFunction(Z)
z_prev = Function(Z)

(u, P, u_b, B, E, j) = split(z)
(ut, Pt, u_bt, Bt, Et, jt) = split(z_test)
(up, Pp, u_bp, Bp, Ep, jp) = split(z_prev)
...
z_prev.sub(3).interpolate(project_ic(B_init))  # B component
...
def spectrum_and_save(u, B, tval,
    ...
    """
    计算谱并保存为 CSV，增加 cross helicity 谱。
    - u, B: Function (2D vector) at current time, 支持 .at([x,y])
    - tval: 当前时间（float）
    - save_dir, aggregate_filename, per_timestep_files: 同原来
    返回：k (1..kmax-1), E_u[1:], E_B[1:], H_cross[1:]
    注意：保持原有 FFT / 归一化 约定（未除以 N**2 或体积）。
    """
    # 只让主进程写文件
    rank = mesh.comm.rank
    if rank != 0:
    ...
    # 采样并做 FFT
    u_vals = np.zeros((N, N, 2), dtype=float)
    B_vals = np.zeros((N, N, 2), dtype=float)

    for i in range(N):
        xi = x[i]
        for j in range(N):
            yj = y[j]
            u_vals[i, j, :] = u.at([xi, yj])
            B_vals[i, j, :] = B.at([xi, yj])

    # 组件 FFT（注意：二维场，但仍用 fftn）
    ...
    # cross helicity per mode: Re( û · conj(B̂) )
    H_cross_mode = np.real(uhat_x * np.conj(Bhat_x) + uhat_y * np.conj(Bhat_y))

    # 壳累加
    kmax = int(np.max(K)) + 1
    E_u = np.zeros(kmax, dtype=float)
    E_B = np.zeros(kmax, dtype=float)
    H_cross = np.zeros(kmax, dtype=float)
    ...
    for kk in range(kmax):
        ...
        if np.any(mask):
            ...
            H_cross[kk] = np.sum(H_cross_mode[mask])
        else:
            # 没有模式也保持 0
            E_u[kk] = 0.0
            E_B[kk] = 0.0
    ...
    if per_timestep_files:
        ...
        with open(fname, "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(["k", "E_u", "E_B", "H_cross", "E_total", "t"])
            for kk in k_arr:
    ...
    # 返回便于进一步处理
    return k_arr, E_u[1:], E_B[1:], H_cross[1:]

F = (
    # u
     inner((u - up)/dt, ut) * dx
    #+ inner(filter_term(u_avg, u_b), ut) * dx # correction term
    - inner(vcross(u_b_avg, curl(u_avg)), ut) * dx # advection term
    ...
    # E
    + inner(E_avg, Et) * dx
    + inner(scross(u_b_avg, B_avg), Et) * dx
...
(u_, P_, u_b_, B_, E_, j_) = z.subfunctions
u_.rename("Velocity")
P_.rename("Pressure")
...
pvd = VTKFile("output/mhd-alpha-removeH.pvd")
...
energy = energy_uB(z_prev.sub(0),u_b, z_prev.sub(3)) #u, u_b, B
crosshelicity = helicity_c(z.sub(0), z.sub(3)) # u, u_b, B
maghelicity = helicity_m(z.sub(3)) # B
divu = div_u(z.sub(0))
divB = div_B(z.sub(3))
# monitor
w = Function(Q).interpolate(scurl(z.sub(0)))
w_max, j_max, ens_total = compute_ens(w, z.sub(5)) # w, j
...
while (float(t) < float(T-dt)+1.0e-10):
    ...
    energy = energy_uB(z.sub(0),u_b, z.sub(3)) #u, u_b, B
    crosshelicity = helicity_c(z.sub(0), z.sub(3)) # u, u_b, B
    maghelicity = helicity_m(z.sub(3)) # B
    divu = div_u(z.sub(0))
    divB = div_B(z.sub(3))
    # monitor
    w = Function(Q).interpolate(scurl(z.sub(0)))
    w_max, j_max, ens_tol = compute_ens(w, z.sub(5)) # w, j
