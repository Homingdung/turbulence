"""
3D MHD Energy Spectrum Evolution Animation
--------------------------------------------
Panels: E_u(k), E_B(k), H_mag(k), E_total(k), H_cross(k)
Layout: 2×3  (last cell used for time readout)
Background: white, publication-friendly

Usage:
    python turbulence_spectrum_3d.py
...
# ── Load data ──────────────────────────────────────────────────────────────────
df = pd.read_csv('output/spectrum_all.csv')
...
E_u     = build_array('E_u')
E_B     = build_array('E_B')
H_mag   = build_array('H_mag')
E_tot   = build_array('E_total')
...
for ax in [ax_u, ax_B, ax_tot]:
    ax.loglog(k_ref, k53, color=C_ref, lw=1.2, ls='--', alpha=0.7,
              label=r'$k^{-5/3}$', zorder=1)

# H_mag and H_cross can be negative — use semilogx
ax_hm.axhline(0, color=C_ref, lw=1.2, ls='--', alpha=0.7, zorder=1)
...
ln_hm,  = ax_hm.plot(   [], [], color=C_hm,  lw=2.0, zorder=3)
...
ax_hm.set_xlabel('$k$', **lkw);  ax_hm.set_ylabel(r'$H_{mag}(k)$',     **lkw)
...
ax_hm.set_title('Magnetic Helicity',          **tkw)
...
fig.suptitle('3D MHD Turbulence — Energy Spectrum Evolution',
...
for ax, arr in [(ax_u, E_u), (ax_B, E_B), (ax_tot, E_tot)]:
    ax.set_xlim(k[0] * 0.9, k[-1] * 1.1)
    ax.set_ylim(*ll_lim(arr))

# H_mag and H_cross: semilogx with symmetric y limits
ax_hm.set_xscale('log')
ax_hm.set_xlim(k[0] * 0.9, k[-1] * 1.1)
hm_abs = np.abs(H_mag).max()
ax_hm.set_ylim(-hm_abs * 1.2, hm_abs * 1.2)

ax_hc.set_xscale('log')
ax_hc.set_xlim(k[0] * 0.9, k[-1] * 1.1)
...
try:
    ...
    ani.save('spectrum_evolution_3d.mp4', writer=writer, dpi=130,
             savefig_kwargs={'facecolor': 'white'})
    print("✓  Saved  spectrum_evolution_3d.mp4")
except Exception:
    print("ffmpeg not found — saving as GIF instead...")
    ani.save('spectrum_evolution_3d.gif',
             writer='pillow', fps=15, dpi=100,
             savefig_kwargs={'facecolor': 'white'})
    print("✓  Saved  spectrum_evolution_3d.gif")
