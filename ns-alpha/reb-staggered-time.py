# helicity-conservation test
# see whether the staggered time step affects the conservation
from firedrake import *
import csv

def helicity_u(u):
    return assemble(inner(u, curl(u))*dx)

def energy_u(u):
    return 0.5 * assemble(inner(u, u) * dx)
    
def div_u(u):
    return norm(div(u), "L2")

# solver parameter
lu = {
    "mat_type": "aij",
    "snes_type": "newtonls",
    "ksp_type": "preonly",
    "pc_type": "lu",
    "pc_factor_mat_solver_type": "mumps",
}
sp = lu

# spatial parameters
baseN = 4
nref = 0

# temporal parameters
mesh = PeriodicUnitCubeMesh(baseN, baseN, baseN)
x, y, z0 = SpatialCoordinate(mesh)

# spatial discretization
Vg = VectorFunctionSpace(mesh, "CG", 2)
Q = FunctionSpace(mesh, "CG", 1)
Vc = FunctionSpace(mesh, "N1curl", 1)
Vg_ = FunctionSpace(mesh, "CG", 1)

# time 
t = Constant(0) 
T = 1.0
dt = Constant(0.025)

# (u, p, w)
Z = MixedFunctionSpace([Vg, Q, Vg])
z = Function(Z)
z_test = TestFunction(Z)
z_prev = Function(Z)
z_mean = Function(Z)

(u, p, w) = split(z)
(ut, pt, wt) = split(z_test)
(up, pp, wp) = split(z_prev)
(u_mean, p_mean, w_mean) = split(z_mean)

# initial condition
nu = Constant(0)
alpha = CellSize(mesh)

u_init = as_vector([cos(2*pi*z0), sin(2*pi*z0), sin(2*pi*x)])
w_init = curl(u_init)
z_prev.sub(0).interpolate(u_init)
#z_prev.sub(2).interpolate(w_init)

z.assign(z_prev)

u_avg = (u + up)/2
w_avg = w
p_avg = p

F = (
    # u
      inner((u - up)/dt, ut) * dx
    - inner(cross(u_avg, w_avg), ut) * dx
    + inner(grad(p_avg), ut) * dx
    # p
    + inner(u_avg, grad(pt)) * dx
    # w
    + inner(w_avg, wt) * dx
    + inner(curl(u_avg), wt) * dx
)

(u_, p_, w_) = z.subfunctions
u_.rename("Velocity")
p_.rename("Pressure")
w_.rename("Vorticity")

pvd = VTKFile("output/reb-sp.pvd")
pvd.write(*z.subfunctions, time = float(t))
bcs = None
pb = NonlinearVariationalProblem(F, z, bcs)
solver = NonlinearVariationalSolver(pb, solver_parameters = sp)

data_filename = "output/data.csv"
fieldnames = ["t", "divu", "energy", "helicity"]

if mesh.comm.rank == 0:
    with open(data_filename, "w", newline='') as f:
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        writer.writeheader()

energy = energy_u(z.sub(0))
helicity = helicity_u(z.sub(0))
divu = div_u(z.sub(0))

if mesh.comm.rank == 0:
    row = {
        "t": float(t),
        "divu": float(divu),
        "energy": float(energy),
        "helicity": float(helicity),
    }
    with open(data_filename, "a", newline='') as f:
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        writer.writerow(row)

timestep = 0
while (float(t) < float(T-dt)+1.0e-10):
    t.assign(t+dt)
    if mesh.comm.rank == 0:
        print(GREEN % f"Solving for t = {float(t):.4f}, dt = {float(dt)}, T = {T}, baseN = {baseN}, nref = {nref}, nu = {float(nu)}", flush=True)
    solver.solve()
    z_mean.assign((z + z_prev)/2) 

    energy = energy_u(z.sub(0))
    helicity = helicity_u(z_mean.sub(0))
    divu = div_u(z.sub(0))

    if mesh.comm.rank == 0:
        row = {
            "t": float(t),
            "divu": float(divu),
            "energy": float(energy),
            "helicity": float(helicity),
        }
        with open(data_filename, "a", newline='') as f:
            writer = csv.DictWriter(f, fieldnames=fieldnames)
            writer.writerow(row)

    print(row) 
    pvd.write(*z.subfunctions, time=float(t))
    timestep += 1
    z_prev.assign(z)

